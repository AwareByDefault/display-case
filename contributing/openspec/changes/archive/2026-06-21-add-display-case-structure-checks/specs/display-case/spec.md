...
### Requirement: Placard presence and use

When its rule is enabled, the structure checks SHALL verify that the showcase provides a placard that is configured, exists, embeds at least one live specimen using the specimen contract, and is not effectively empty. A placard that is unconfigured, absent, contentless, or that embeds no specimen using the specimen contract SHALL be reported. Disabling this rule SHALL suppress these reports, consistent with a placard being optional.

#### Scenario: No placard configured

- GIVEN a showcase that configures no placard
- AND the placard rule is enabled
- WHEN the placard check runs
- THEN the showcase is reported as missing a placard

#### Scenario: Placard configured but empty

- GIVEN a showcase that configures a placard that exists but is effectively empty or embeds no specimen using the specimen contract
- WHEN the placard check runs
- THEN the placard is reported as present but unused

#### Scenario: Placard present and used

- GIVEN a showcase that configures a placard that exists, is non-trivial, and embeds at least one live specimen using the specimen contract
- WHEN the placard check runs
- THEN the placard produces no violation

#### Scenario: Placard rule disabled

- GIVEN a showcase that configures no placard
- AND the placard rule is disabled
- WHEN the structure checks run
- THEN no placard violation is reported
