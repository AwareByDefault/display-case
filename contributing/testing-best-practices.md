...
**5.5 Committed visual baselines must match the CI render environment.** `display-case.config.ts` sets `baselineDir: ./test/visual-baselines` (committed). Pixel renders differ across OSes, so record baselines in the *same* image CI diffs in. Two ways to refresh after an intentional visual change:

- `bun run baselines:record` — records inside the pinned `mcr.microsoft.com/playwright` Docker image locally (the `visual` CI job runs in that same image). Review the PNGs and commit.
- The **Update visual baselines** workflow (`.github/workflows/update-baselines.yml`, `workflow_dispatch`) — records in the CI container and commits the baselines back to the chosen branch. Use when you lack Docker locally.

Never `--update` on macOS and commit — CI (Linux) would never match. Because the committed baselines are Linux-recorded, the config also opts `visual` out of the *default* run (`check.defaultPhases.visual = false`) so a bare `display-case check .` on a non-Linux machine doesn't report false diffs; `--visual` and CI still run it. Keep the image tag (in both `ci.yml` and `update-baselines.yml`), the record script, and the `playwright` version in `bun.lock` in lockstep.
...
**11.4 Validate client hydration in a real browser, not just SSR output.** When verifying a bundling/externalization change (sharing or externalizing a runtime lib via `config.share`, a `pinReact` tweak, a vendor-chunk experiment), the SSR'd markup can mask a *dead client bundle* — the page looks right because the server rendered it, while hydration is silently broken. So check a **browser-only** case (one with no SSR mask) in a real browser; a missing named export (`react does not provide an export named 'StrictMode'`) or an invalid-hook-call only surfaces there. Pure-markup cases never catch it — use a hook-using fixture. (This is the lesson the reverted shared-React-vendor spike taught; see [NOTES.md](NOTES.md).)
