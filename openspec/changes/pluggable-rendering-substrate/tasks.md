...
- [x] 4.2 Delegate the a11y audit to `substrate.checks.audit` with not-applicable reporting for substrates that supply none
- [x] 4.3 Delegate capture to `substrate.checks.capture` (default `serialize(render(...))`); the DOM substrate implements capture via its browser driver; substrate `diff` is the default under consumer overrides (design D4)
- [x] 4.4 Re-key baselines to `<baselineDir>/<substrate.id>/<component>/<case>.<variantKey>.<ext>` with the DOM substrate reading the legacy flat path for one release and printing a migration hint (design D6)
