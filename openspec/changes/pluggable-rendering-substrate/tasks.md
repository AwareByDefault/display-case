...
- [ ] 3.1 Implement axis declarations with the render/stage kind split; `domSubstrate()` declares the theme axis (render) and the viewport-width axis (stage) with today's ids and values (design D6)
...
- [ ] 3.3 Carry the substrate id and declared axes in the manifest; update manifest tests
- [ ] 3.4 Accept declared render-axis parameters at the render endpoint (the theme parameter stays valid for the DOM substrate); shell and primer documents follow
