...
Display Case SHALL deliver each isolated case render and the placard as a document whose content is already rendered — laid out and themed for the requested theme — before any of the page's own scripts execute. A client that retrieves such an address and does not execute its scripts SHALL still receive that address's content, not an empty shell. Interactive behaviors of those surfaces (theme switching, viewport changes, tweak controls, placard navigation) MAY depend on the page's scripts, but the initial rendered content SHALL NOT. The content delivered before scripting SHALL be the same content the surface produces once interactive, so that themes, snapshots, and addresses are unchanged.
...
- WHEN the page's scripts run and a viewer switches theme, changes the viewport width, adjusts a tweak, or navigates the placard
