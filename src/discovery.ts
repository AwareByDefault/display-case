...
export async function codegenSsrEntry(
  ...
  const here = resolve(import.meta.dir)
  const rendererImport = importPath(entry, join(here, 'ssr-render.tsx'))
  const configImport = importPath(entry, configPath)
...
export async function codegenSsrPrimerEntry(
  ...
  const here = resolve(import.meta.dir)
  const rendererImport = importPath(entry, join(here, 'ssr-primer.tsx'))
  const mdxImport = importPath(entry, resolve(pkgDir, primerPath))
...
export const renderPrimerToHtml = makePrimerRenderer(MDXContent)
`
  await Bun.write(entry, source)
  return entry
}
