...
export async function codegenCaseSsrEntry(
  ...
  const substrateImport = importPath(
    entry,
    join(here, 'substrate', 'resolve.ts'),
  )
  const configImport = importPath(entry, configPath)
  const modImport = importPath(entry, file)

  // The entry resolves the substrate and wires it into the renderer. Doing it
  // here — rather than inside `ssr-render` — keeps the render layer pointing
  // inward: it depends on the substrate *contract*, never on an implementation.
  const source = `// AUTO-GENERATED by display-case — do not edit.
import { makeCaseRenderer } from '${rendererImport}'
import { resolveSubstrate } from '${substrateImport}'
import config from '${configImport}'
import m0 from '${modImport}'

export const renderCaseToHtml = makeCaseRenderer([Object.assign(m0, { sourcePath: ${JSON.stringify(relative(pkgDir, file))} })], config, resolveSubstrate(config))
`
  await Bun.write(entry, source)
  return entry
}
...
/**
 * Codegen the published build's substrate entry: a module exporting the
 * showcase's resolved substrate and its config.
 *
 * The production host serves each isolated render through `substrate.document()`
 * exactly as the dev server does, so it needs the substrate the showcase
 * configured — not a hard-coded assumption that documents are HTML. Bundling it
 * here (rather than resolving the config at serve time) keeps the published
 * build self-contained: it carries no reference to the consumer's source tree.
 */
export async function codegenSubstrateEntry(
  pkgDir: string,
  configPath: string,
): Promise<string> {
  const dir = cacheDir(pkgDir)
  const entry = join(dir, 'substrate-entry.ts')
  const here = resolve(import.meta.dir, '..')
  const resolveImport = importPath(entry, join(here, 'substrate', 'resolve.ts'))
  const configImport = importPath(entry, configPath)

  const source = `// AUTO-GENERATED by display-case — do not edit.
import { resolveSubstrate } from '${resolveImport}'
import config from '${configImport}'

export { config }
export const substrate = resolveSubstrate(config)
`
  await Bun.write(entry, source)
  return entry
}
