...
/** The React entry points the browser bundles import (and the vendor re-exports),
 *  in priority order — earlier wins a name collision (e.g. `Fragment`, shared by
 *  `react` and `react/jsx-runtime`; `version`, shared by all). */
const VENDOR_MODULES: ReadonlyArray<readonly [spec: string, ns: string]> = [
  ['react', '_react'],
  ['react-dom', '_reactDom'],
  ['react-dom/client', '_client'],
  ['react/jsx-runtime', '_jsx'],
]

/**
 * Codegen the shared **vendor** entry: re-exports the consumer's React so the
 * chrome and every per-component bundle can import it as an external bare specifier
 * (resolved by an importmap to this one content-hashed bundle) rather than inlining
 * its own copy. Built with `pinReact`, so the single consumer React copy lands here
 * and every externalized bundle shares it — no dual React, one download.
 *
 * Why introspect-and-re-export rather than `export * from 'react'`: React ships as
 * CommonJS. `export *` over CJS yields runtime property *copies*, not the static
 * ESM export bindings that another module's `import { useState } from 'react'`
 * (left external, resolved here via the importmap) needs at link time — those throw
 * "Export … is not defined" in the browser. A *named* `export { createRoot }` gives
 * a static binding but lets the bundler tree-shake the reconciler it can't see
 * `createRoot` reach through CJS. The robust form, generated here per build from the
 * *actually installed* exports (so it tracks the consumer's React version): import
 * each module as a namespace (which retains its whole impl — no tree-shaking) and
 * re-export every name as `export const x = ns.x` (a static binding). First module
 * wins a duplicate name.
 */
async function codegenVendorEntry(pkgDir: string): Promise<string> {
  const imports: string[] = ['// AUTO-GENERATED by display-case — do not edit.']
  const exportLines: string[] = []
  const seen = new Set<string>(['default'])
  for (const [spec, ns] of VENDOR_MODULES) {
    imports.push(`import * as ${ns} from '${spec}'`)
    ...
    for (const key of Object.keys(mod)) {
      ...
      seen.add(key)
      exportLines.push(`export const ${key} = ${ns}.${key}`)
    }
  }
  // React's default (the React namespace object) — `import React from 'react'`.
  exportLines.push('export default _react.default')
  const entry = join(cacheDir(pkgDir), 'vendor-react.ts')
  await Bun.write(entry, [...imports, '', ...exportLines, ''].join('\n'))
  return entry
}
...
export interface BuildDescriptor {
   ...
   *  `vendor` is the one shared React bundle every browser entry imports (via an
   *  importmap) instead of inlining its own copy. */
  assets: {
    browser: string
...
export async function publish(
  ...
  // React is shipped ONCE as a shared vendor bundle and imported by the chrome and
  // every per-component bundle as external bare specifiers, resolved in the browser
  // by an importmap (see documents.ts). Without this, each per-component browser
  // bundle inlines its own ~150 KB React copy — N components meant N copies on the
  // published site. The vendor build itself bundles React (pinReact pins the
  // consumer's single copy); the externalized builds carry none.
  ...
  const vendorEntry = await codegenVendorEntry(pkgDir)
  const vendorOut = await runPublishBuild('the shared React vendor bundle', {
    pkgDir,
  ...
  const vendorEp = vendorOut.find((o) => o.kind === 'entry-point')
  if (vendorEp) assets.vendor = `${base}/assets/${basename(vendorEp.path)}`
    ...
    components.map(async (c) => {
      ...
      const outs = await runPublishBuild(
        `component "${c.id}" (${relative(pkgDir, c.file)})`,
        {
          ...
          naming: Hashed,
          define: defines,
          // React external → the shared vendor bundle (importmap), not inlined per
          // component. This is the bundle-size win: ~150 KB React once, not ×N.
          external: browserExternal,
          pinReact: false,
        },
      )
