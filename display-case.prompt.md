...
- **Hierarchy `level`** (third arg `{ level }`), ordered `atom → molecule → organism → template → page → flow`. It classifies every component and groups the **Components** browse mode (the kit, atom–template); omit it and the component lands in "Unclassified". Pages and flows are **surfaces** — browsed in the separate **Exhibits** mode, grouped by IA, not by level.
- **IA `group`** (third arg `{ group }`; also `defineFlow(name, { group, steps })`) places a `page`/`flow` surface in the Exhibits nav tree, as a path (`'App/Settings'` or `['App','Settings']`). Omit to derive it from the case file's folder (then config, then a default group). A *navigation* concern — distinct from `area`.
- **App chrome `area`** (third arg `{ area }`; also `defineFlow(name, { area, steps })`) is a free-form tag the package's `decorator` can use to wrap a `page`/`flow` case in app layout/nav. The decorator interprets it (Display Case mandates no values); it overrides folder-based detection via `sourcePath`. Omit to render bare. A *rendering* concern — distinct from `group`.
